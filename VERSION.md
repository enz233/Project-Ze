...
## Unreleased
- ASR Qwen-ASR 实时识别：新增专用 `qwen-asr-realtime` WebSocket 引擎，使用 Workspace ID 专属域名和握手 `Authorization` 请求头，不再通过 OpenAI-compatible `/audio/transcriptions` 路径调用 Qwen-ASR；补充 `docs/qwen-asr-configuration.md` 配置说明，并将 final/completed 事件等待窗口延长到 15 秒，避免服务端最终结果稍晚返回时被过早关闭。
