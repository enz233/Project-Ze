...
| Screen Target Pointer | `.` 显式目标指示中定位目标、移动到目标旁，并使用八方向 point 姿态指向目标 | ✔ |
| Voice Input (ASR) | 麦克风按钮 + 长按快捷键，支持 OpenAI-compatible 与 Qwen-ASR 实时识别配置 | ✔ |
...
ASR 供应商在 F11 设置的“语音输入”页配置。OpenAI、阿里百炼 / DashScope、自定义 OpenAI-compatible 预设走 OpenAI-compatible ASR 引擎；Qwen-ASR 实时识别请选择专用 `Qwen-ASR 实时识别` 预设，填写 Workspace ID、API Key 和模型，详细配置见 [docs/qwen-asr-configuration.md](docs/qwen-asr-configuration.md)。普通模式保留供应商预设、Base URL、Workspace ID、API Key 和模型，高级模式再调整实际引擎、路径、流式模式和缓存。
...
**Unreleased / Next**
- [ ] Multimodal response workflow orchestrator
