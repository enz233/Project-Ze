...
> 本文档供 AI 助手快速了解项目结构，避免每次读全部代码。最后更新：v0.2.15
...
- **主动回应**：ObserverManager → ProactiveReactionSystem → BubbleManager，基于应用切换、工作/休息转换、长专注和直接互动生成轻柔回应；Debug 面板显示最近决策/拦截原因/预算状态
...
- **情境化主动回应**：本地规则先判断是否应该回应，AI 仅用于高价值场景短句改写，不用于决定是否打扰；v0.2.15 放宽部分阈值并加入可视化调试快照
