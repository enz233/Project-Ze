...
如果连接、录音和结束流程都显示成功，但服务端没有返回 `conversation.item.input_audio_transcription.text` 或 `conversation.item.input_audio_transcription.completed`，项目现在会显示可操作错误：
...
3. 如果音量正常但仍无文本，优先怀疑浏览器 `MediaRecorder` 产出的 `audio/webm;codecs=opus` 是否被当前 Qwen-ASR 模型接受；后续可按阿里云模型要求改为更明确的 PCM/WAV 采集链路。
4. 查看日志中的 Qwen-ASR 服务端错误事件，确认是否返回音频格式、模型或权限相关错误。
