...
3. 设置页的 Qwen-ASR 识别测试会绕过浏览器 `MediaRecorder` 的 `audio/webm;codecs=opus`，改用 Web Audio 采集并发送 `audio/pcm;rate=16000` PCM16 小端音频，避免实时 ASR 模型因 webm/opus 不兼容而只结束会话、不返回文本。
