...
export interface CameraAwarenessBackgroundRunnerPorts {
  ...
  requestFrame(): Promise<CameraFrameInput>;
...
export class CameraAwarenessBackgroundRunner {
  ...
  constructor(
    ...
    this.clearTimer = ports.clearTimer ?? ((timer) => clearTimeout(timer as ReturnType<typeof setTimeout>));
  }
  ...
  async runOnce(): Promise<void> {
    ...
    this.inFlight = true;
    try {
      const frame = await this.ports.requestFrame();
      await this.ports.processFrame(frame);
    } catch (error: any) {
      this.ports.recordError(toError(error));
    } finally {
      this.inFlight = false;
      if (this.running) {
        this.scheduleNext();
